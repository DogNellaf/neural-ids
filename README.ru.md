...
NeuralIDS — десктопное приложение для Windows, которое в реальном времени перехватывает сетевой трафик, извлекает признаки потоков и классифицирует их нейронной сетью (Keras/TensorFlow). Обнаруженные атаки отображаются в графическом интерфейсе (PyQt5) и сохраняются в базе данных PostgreSQL.

## Возможности

- Захват сетевого трафика в реальном времени через Scapy (`AsyncSniffer`)
- Извлечение признаков потоков (80+ признаков на поток)
- Классификация нейросетью (Keras/TensorFlow) по 15 классам атак
- Графический интерфейс (PyQt5) со списком инцидентов и возможностью их скрытия
- Сохранение обнаруженных атак в PostgreSQL
- Экспорт записей об атаках в CSV
- Блокировка соединений через Windows Firewall (`New-NetFirewallRule`)
...
|---|---|
| Язык | Python 3.10+ |
| Захват трафика | Scapy (`AsyncSniffer`), Npcap/WinPcap |
| Машинное обучение | Keras / TensorFlow, scikit-learn (`StandardScaler`) |
| GUI | PyQt5 |
...
- Python 3.10+
- PostgreSQL 14+
- Windows (захват трафика через Scapy и блокировка через Windows Firewall)
